...
class PetEventRepository extends Repository {
    ...
    public function getEvents(int $userId): array {
        $sql = "SELECT 'event' as type, event_id as id, pet_name, picture_url, title, date, time
                FROM v_pet_events_calendar WHERE user_id = :userId ORDER BY date ASC, time ASC";
    ...
    public function getEventById(int $id): ?array { return $this->fetchById('global_events', $id); }
    public function deleteEvent(int $id): void { $this->deleteRow('global_events', $id); }
